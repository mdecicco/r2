class OtherTestState extends engine.State {
	...
	willBecomeActive = () => {
		engine.log("OtherTestState willBecomeActive", this);
	}
	...
	render = () => {
		if (this.windowOpen) {
			ImGui.Begin("Test Window", () => { this.windowOpen = false; }, [ImGui.WindowFlags.None]);
				...
				if (ImGui.Button("Set State", ImGui.ImVec2(0.0, 0.0))) {
				...
				ImGui.PlotLines("##actfreq", this.fps_history, 0, `Actual Frequency (~${this.frq_avg.average.toFixed(1)} Hz)`, 0, 120, ImGui.ImVec2(400, 200));
				ImGui.PlotLines("##maxfreq", this.aud_history, 0, `Max Frequency (~${this.max_freq_avg.average.toFixed(1)} Hz)`, 0, 120, ImGui.ImVec2(400, 200));
			ImGui.End();
		}
		if (this.logWindowOpen) {
			ImGui.Begin("Log Window", () => { this.logWindowOpen = false; }, [ImGui.WindowFlags.None]);
				const color = ImGui.ImVec4(0.5, 0.5, 0.5, 1);
				this.logs.forEach(l => {
