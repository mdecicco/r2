...
namespace r2 {
	...
	mesh_construction_data::mesh_construction_data(vertex_format* vtx_fmt, index_type idx_typ, const instance_format* ins_fmt)  {
		m_vertexFormat = vertex_format(*vtx_fmt);
		m_indexType = idx_typ;
		m_instanceFormat = instance_format(*ins_fmt);
		m_vertices = m_indices = m_instances = 0;
	...
	void mesh_construction_data::append_vertex(void* vdata) {
	...
	void mesh_construction_data::append_index(void* idata) {
	...
	void mesh_construction_data::append_instance(void* idata) {
